...
# based on csp_eventlet by Michael Carter <http://github.com/mcarter/csp_eventlet>
#
# This CSP implementation provides a socket object that allows you to use existing
# gevent network code, but listen for CSP connections in addition to TCP/IP. There
# is no data framing; socket semantics are emulated.
#
# An example can be found at <http://dl.dropbox.com/u/1213413/code/csp_gevent.zip>
# - run example.py (Paste is required)
# - navigate to http://localhost:8000/ and read instructions on page
#
# MIT License

import gevent, gevent.queue, gevent.pywsgi, gevent.coros, cgi, logging, uuid, base64
try:
	import json
except:
	import simplejson as json

logger = logging.getLogger('csp_gevent')
...
class Listener(object):
	def __init__(self, interface=None, port=None):
		...
		self.port = port
		self._accept_channel = gevent.queue.Queue(0)
		self._sessions = {}
		
	def listen(self):
		gevent.pywsgi.WSGIServer((self.interface, self.port),self).start()
...
#			raise
	...
	def render_handshake(self, session, environ, start_response):
		...
		gevent.spawn(self._accept_channel.put, (session._socket, ("", 0)))
...
class CSPSession(object):
	...
	def __init__(self, parent, key, environ):
		...
		self._comet_request_lock = gevent.coros.Semaphore(1)
		self._comet_request_channel = gevent.queue.Queue(0)
		self._comet_request_channel_getting = 0
		self._activity_queue = gevent.queue.Queue()
		self._activity_queue_getting = 0
		...
		gevent.spawn(self._timeout, False)
	...
	def _timeout(self, is_teardown):
		while True:
			...
			self._activity_queue_getting += 1
			try:
				if self._activity_queue.get(timeout=self.conn_vars['du'] * 2 or 60):
					break
				continue
	...
	def close(self):
		...
		if self._has_comet_request():
			self._comet_request_channel.put(None)
		if self._activity_queue_getting:
			self._activity_queue.put(True)
		gevent.spawn(self._timeout, True)
		if not self.buffer:
			self._read_queue.put(None)
	...
	def _has_comet_request(self):
		return bool(self._comet_request_channel_getting)
	...
	def comet_request(self, environ, start_response):
		if not self.packets:
			...
			if duration:
				self._comet_request_channel_getting += 1
				try:
					self._comet_request_channel.get(timeout=duration)
				except gevent.queue.Empty, e:
					pass
				finally:
					self._comet_request_channel_getting -= 1
