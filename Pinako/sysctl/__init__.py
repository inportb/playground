...
_sysctl = cdll.LoadLibrary(find_library('c')).sysctl
res = c_uint64(0)
resref = byref(res)
sz = c_size_t(sizeof(res))
szref = byref(sz)
zerosz = c_size_t(0)
def sysctl(*args):
	n = len(args)
	name = (c_int*n)(*args)
	result = _sysctl(name,n,resref,szref,None,zerosz)
	if result != 0:
		raise error('sysctl returned with error %s'%result)
	return res.value

##### SYSCTL CONSTANTS #####
# from /usr/include/linux/sysctl.h
